...
import logo from "../assets/logo.png"; // Fixed the import for the logo
                ...
                <div className='rounded-lg backdrop-blur-md lg:hidden'>
                    <div className='flex items-center justify-between'>
                        ...
                        <button onClick={toggleMobileMenu} className='text-white'>
                            {isMobileMenuOpen ? <FaTimes className='m-2 h-6 w-5' /> : <FaBars className='m-2 h-6 w-5'/>} {/* Toggle button */}
                        </button>
                    </div>
                    {isMobileMenuOpen && (
                        <ul className='flex flex-col items-center mt-4 ml-4 gap-4 backdrop-blur-md'>
                            {NAVIGATION_LINKS.map((item, index) => (
                                <li key={index}>
                                    <a 
                                        href={item.href} 
